...
/*   Updated: 2022/11/30 22:14:06 by rpoder           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

static int	handle_key_event(int keycode, t_data *data)
{
	if (keycode == 65307)
...
void	start_mlx(t_data *data)
{
	...
	data->mlx_data->win = mlx_new_window(data->mlx_data->mlx,
			CANVAS_X, CANVAS_Y, "Hello Zobi !");
	if (!data->mlx_data->win)
		global_free(data, MLX_ERR);
	data->mlx_data->image.img = mlx_new_image(data->mlx_data->mlx,
			CANVAS_X, CANVAS_Y);
	if (!data->mlx_data->image.img)
		global_free(data, MLX_ERR);
	data->mlx_data->image.addr = mlx_get_data_addr(data->mlx_data->image.img,
			&data->mlx_data->image.bits_per_pixel,
			&data->mlx_data->image.line_length, &data->mlx_data->image.endian);
	if (!data->mlx_data->image.addr)
		global_free(data, MLX_ERR);
	mlx_hook(data->mlx_data->win, 17, 0L, handle_mouse_event, data);
	mlx_hook(data->mlx_data->win, 2, 1 << 0, handle_key_event, data);
}
