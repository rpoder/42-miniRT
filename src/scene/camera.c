...
/*   Updated: 2022/11/25 16:36:18 by mpourrey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera	*create_camera(t_data *data, int x_size, int y_size, t_parsing_values_tool values)
{
	...
	if (!data->world->camera)
		return (NULL);
	data->world->camera->vsize = x_size;
	data->world->camera->hsize = y_size;
	data->world->camera->fov = values.fov;
	data->world->camera->pixel_size = compute_pixel_size(data->world->camera, x_size, y_size, values.fov);
